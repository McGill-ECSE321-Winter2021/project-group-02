	...
	<div id="workspace-page">
		<div id="workspace-container" class="v-container">
			...
			<div id="workspace-option" class="button-container">
				...
				>
					<option value="Select a Workspace" disabled selected
				...
				<button id="new-workspace" class="text-button" @click="newWorkspace()">
					New Workspace
				</button>
			</div>
			...
			<div id="new-container" class="v-container" v-if="this.new === true">
				<form class="v-container" @submit.prevent="createWorkspace()">
					...
					<div class="button-container">
						<button class="text-button" @click="cancelWorkspace()">
							Cancel
						</button>
				...
				<div id="form-container" class="form">
					<form class="form" @submit.prevent="updateButton()">
						<div class="button-container">
							...
							<button class="text-button" @click="deleteWorkspace()">
								Delete
							</button>
							<div class="button-spacer" />
					...
					<form class="form" @submit.prevent="createTimeslot()">
						...
						<div class="button-container">
							...
							<select
								id="year-select"
								class="timeslot-select"
								v-model="timeslotForm.year"
								@change="setDate()"
							>
								...
								<option
									...
									>{{ index + curYear - 1 }}</option
								>
							</select>
							<select
								id="month-select"
								class="timeslot-select"
								v-model="timeslotForm.month"
								@change="setDate()"
							>
						...
						<div class="button-container">
							...
							<select
								...
								class="timeslot-select"
								v-model="timeslotForm.startHour"
								@change="setStartTime()"
							>
							...
							<select
								...
								class="timeslot-select"
								v-model="timeslotForm.startMinute"
								@change="setStartTime()"
							>
								<option value="-1" selected disabled>mm</option>
								<option value="0">00</option>
								<option v-for="index in 3" :key="index" :value="index * 15">{{
									index * 15
								}}</option>
							</select>
						</div>
						...
						<div
							...
							v-if="
								timeslotForm.startHour !== -1 && timeslotForm.startMinute !== -1
							"
						>
							...
							<select
								...
								class="timeslot-select"
								v-model="timeslotForm.endMinute"
								@change="setEndTime()"
							>
								<option value="-1" selected disabled>mm</option>
								<option value="0">00</option>
			...
			<button id="back" class="form-button" @click="backViewDash()">
				Back
			</button>
		</div>
	</div>
	...
	export default {
		...
		methods: {
			...
			async createWorkspace() {
				if (
					this.newWorkspaceName === "" ||
					this.newWorkspaceName === undefined
				) {
				...
				try {
					...
					console.error(error);
					this.errorMsg =
						"Something went wrong creating the workspace. Please try again later";
				}
			},
			...
			async workspaceSelect() {
				...
				try {
					let response = await axios.get(
						proxy.proxy +
							"/api/workspace/availabilities/" +
							this.curWorkspace.workspaceId
					);
					...
					this.cancelWorkspace();
					this.errorMsg =
						"Something went wrong loading workspace. Please try again later";
				}
			},
            ...
            this.errorMsg = "";
					...
					console.error(error);
					this.errorMsg =
						"Something went wrong updating the workspace name. Please try again later";
				}
			},
			...
			async deleteWorkspace() {
				if (this.timeslots.length !== 0) {
					this.errorMsg =
						"Please delete all timeslots before deleting workspace";
					return;
				}
				try {
			...
			setDate() {
				if (
					this.timeslotForm.year === -1 ||
					this.timeslotForm.month === -1 ||
					this.timeslotForm.day === -1
				)
					return;

				this.newStartDate.setFullYear(
					this.timeslotForm.year,
					this.timeslotForm.month - 1,
					this.timeslotForm.day
				);
				this.newEndDate.setFullYear(
					this.timeslotForm.year,
					this.timeslotForm.month - 1,
					this.timeslotForm.day
				);
			...
			setStartTime() {
				if (
					this.timeslotForm.startHour === -1 ||
					this.timeslotForm.startMinute === -1
				)
					return;
				this.newStartDate.setHours(
					this.timeslotForm.startHour,
					this.timeslotForm.startMinute
				);
			},
			...
			setEndTime() {
				...
				if (this.timeslotForm.startHour > this.timeslotForm.endHour) {
					...
					this.timeslotForm.startHour === this.timeslotForm.endHour &&
					this.timeslotForm.startMinute >= this.timeslotForm.endMinute
				) {
					this.timeslotForm.endMinute = -1;
					return;
				}

				this.newEndDate.setHours(
					this.timeslotForm.endHour,
					this.timeslotForm.endMinute
				);
			},
            ...
            this.errorMsg = "";
					...
					console.error(error);
					this.errorMsg =
						"Something went wrong adding the timeslot to workspace. Please try again later";
				}
			},
