...
const testViewTechnicianSchedule = async () => {
  ...
  try {
    ...
    let getTechnicianSchedule = await axios.get(
      backendAddress +
        `/api/technician/viewschedule/${technicianIdToCheck}/${startDate}/${endDate}`
    );
    ...
    if (
      statusCode === 200 &&
      technicianIdToCheck === ScheduleResponseData[0].techniciansId[0]
    ) {
